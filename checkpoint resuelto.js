...
var cardGame = function(playerOneCards, playerTwoCards){
  ...
  var objFinal = {
    winner: "",
    castleOne: 0, //la cantidad de puntos de resistencia finales del player one
    castleTwo: 0 //la cantidad de puntos de resistencia finales del player two
  }
  ...
  do {
    ...
    //* Sacos 2 cartas Player 1
    var cartaPlayer1ataque = playerOneCards.dequeue() //* La primera carta (Player1)será su carta asignada para atacar
    var cartaPlayer1defensa = playerOneCards.dequeue() //* La segunda carta (Player1)será su carta asignada para defender
    
    //* Sacos 2 cartas Player 2
    var cartaPlayer2ataque = playerTwoCards.dequeue() //* La primera carta (Player2)será su carta asignada para atacar
    var cartaPlayer2defensa = playerTwoCards.dequeue() //* La segunda carta (Player2)será su carta asignada para defender
    
    //* ----------------------------
  
    //* PLAYER 1 - Segun que tipo de carta, se incrementa la defensa o el ataque.
    
    if (cartaPlayer1defensa.type === "Protector") { //* si la carta1 de defensa contiene este tipo se dupicarán sus puntos de defensa
      cartaPlayer1defensa.defense = cartaPlayer1defensa.defense * 2
       }
    if (cartaPlayer1ataque.type === "Protector") { //* si la carta1 de ataque contiene este tipo se dupicarán sus puntos de defensa
      cartaPlayer1ataque.defense = cartaPlayer1ataque.defense * 2
    }
    if (cartaPlayer1ataque.type === "Destructor") { //* si la carta2 de ataque contiene este tipo se duplicarán sus puntos de ataque
      cartaPlayer1ataque.attack = cartaPlayer1ataque.attack * 2
    }
    if (cartaPlayer1defensa.type === "Destructor"){ //* si la carta2 de defensa contiene este tipo se duplicarán sus puntos de ataque
      cartaPlayer1defensa.attack = cartaPlayer1defensa.attack * 2
    }
    
    //* PLAYER 2 - Segun que tipo de carta, se incrementa la defensa o el ataque.

    if (cartaPlayer2defensa.type === "Protector") { //* si la carta1 de defensa contiene este tipo se dupicarán sus puntos de defensa
      cartaPlayer2defensa.defense = cartaPlayer2defensa.defense * 2
       }
    if (cartaPlayer2ataque.type === "Protector") { //* si la carta1 de ataque contiene este tipo se dupicarán sus puntos de defensa
      cartaPlayer2ataque.defense = cartaPlayer2ataque.defense * 2
    }
    if (cartaPlayer2ataque.type === "Destructor") { //* si la carta2 de ataque contiene este tipo se duplicarán sus puntos de ataque
      cartaPlayer2ataque.attack = cartaPlayer2ataque.attack * 2
    }
    if (cartaPlayer2defensa.type === "Destructor"){ //* si la carta2 de defensa contiene este tipo se duplicarán sus puntos de ataque
      cartaPlayer2defensa.attack = cartaPlayer2defensa.attack * 2
    }
     
    //* -------------------------------
  
    //* Aplico el Ataque al Castillo del Player 2
  
    var ataquePlayerOne = cartaPlayer2defensa.defense - cartaPlayer1ataque.attack //*La carta asignada para atacar del Player1 se enfrentará contra la carta asignada para defender del Player2 
    if (ataquePlayerOne < 0) { //* Si el ataque supera los puntos de defensa el daño sobrante será aplicado sobre el castillo.
      castleTwo += ataquePlayerOne
    }
  
    //* Aplico el Ataque al Castillo del Player 1
  
    var ataquePlayerTwo = cartaPlayer1defensa.defense - cartaPlayer2ataque.attack //*La carta asignada para atacar del Player2 se enfrentará contra la carta asignada para defender del Player1
    if (ataquePlayerTwo < 0) { //* Si el ataque supera los puntos de defensa el daño sobrante será aplicado sobre el castillo.
      castleOne += ataquePlayerTwo
    }

    if (castleOne < 0 || castleTwo < 0) { //* El juego finaliza cuando alguno de los dos castillos se quede sin puntos de resistencia.
      break
    }
  ...
  if(castleOne < castleTwo) { //* Gana Player1
    objFinal.winner = "PLAYER TWO"
    objFinal.castleOne = castleOne
    objFinal.castleTwo = castleTwo
  } else if (castleOne > castleTwo){ //* Gana Player2
    objFinal.winner = "PLAYER ONE" 
    objFinal.castleOne = castleOne
    objFinal.castleTwo = castleTwo
  } else { //* SINO Empate
    objFinal.winner = "TIE" 
